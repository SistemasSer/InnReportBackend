            ...
            for nit_info in item.get("nit", {}).get("solidaria", []):
                ...
                if entity_data:
                    for result in entity_data:
                        ...
                        saldo = clean_currency_value(valor_en_pesos)

                        transformed_results[key]["saldos"].append(
                            {"periodo": periodo, "mes": mes_number, "saldo": saldo}
                        )
                else:
                    # Solo si no hay resultados en la API, verificamos en la base de datos
                    if query_results:
                ...
                # Si no hay resultados en ninguna fuente, agregar saldo 0
                if not entity_data and not saldo_en_bd:
                    transformed_results[key]["saldos"].append(
                        {"periodo": periodo, "mes": mes_number, "saldo": 0.0}
                    )

        # Limitar la cantidad de saldos a 6
...
class BalCoopApiViewIndicador(APIView):
    def post(self, request):
        ...
        mes_anterior = None
        # print(f"Datos cantidad datos entrada: {data}") 
        ...
        for item in data:
            ...
            # print(f"PERIODO: {periodo}, MES_NUMERO: {mes_number}, MES: {mes}")
            ...
            # print(f"URL: {url_solidaria}")

            all_data = [] 

            # print(f"Datos de la API ANTES DE RESPONSE: {len(all_data)}") 
            ...
            if response_solidaria.status_code == 200:
                ...
                # print(f"Datos de la API DESPUES DEL EXTEND: {len(all_data)}") 

            
            puc_codes_current = [
                    "100000", "110000", "120000", "140000", "210000", 
                    "230000", "240000", "300000", "310000", "311010", 
                    "320000", "330500", "340500", "350000", "415000", 
                    "615005", "615010", "615015", "615020", "615035"
                ]
            ...
            # print(f"Saldos actuales: {saldos_current}")
            ...
            if periodo_anterior == periodo_anterior_actual and mes_anterior == mes_ultimo_str:
                ...
                # Realizar la solicitud para obtener los datos del periodo anterior
                mes_12_anterior = get_month_name(12)  # Mes de diciembre del año anterior
                ...
                # print(f"URL: {url_solidaria_prev}")
                response_solidaria_prev = requests.get(url_solidaria_prev)

                if response_solidaria_prev.status_code == 200:
                    # print(f"Datos de la API_PREV ANTES DE RESPONSE: {len(all_data_prev)}") 
                    all_data_prev = response_solidaria_prev.json()
                    # print(f"Datos de la API_PREV DESPUES DEL EXTEND: {len(all_data_prev)}") 
                else:
            ...
            # print(f"Saldos anteriores: {saldos_previous}")
            ...
            for nit_info in item.get("nit", {}).get("solidaria", []):
                ...
                try:
                    ...
                    # print(f"referencia de las entidades: {formatted_nit_dv} - {razon_social}") 

                    # print(f"Saldo indicador_cartera:() {get_saldo(formatted_nit_dv, razon_social, "140000", saldos_current)} / {get_saldo(formatted_nit_dv, razon_social, "100000", saldos_current)} * 100)") 
                    ...
                    # print(f"Saldo denominator_roe:({get_saldo(formatted_nit_dv, razon_social, "300000", saldos_previous)} + {get_saldo(formatted_nit_dv, razon_social, "300000", saldos_current)} / {mes_decimal} * 12) / 2") 
...
class BalCoopApiViewBalance(APIView):
    def post(self, request):
        ...
        # print(f"Processing periodo: {periodo}, mes: {mes}")
        ...
        if response_Solidaria.status_code == 200:
            ...
            # print(f"Fetched {len(all_data)} records for periodo: {periodo}, mes: {mes}")

            # Agrupar datos de la API
            for result in all_data:
                nit = result.get("nit")
                valor_en_pesos = result.get('valor_en_pesos', '$ 0')
                total_saldo = clean_currency_value_Decimal(valor_en_pesos)
        ...
        # Consultar la base de datos para entidades no encontradas en la API
        for nit_info in entidades_Solidaria:
            razon_social = nit_info.get("RazonSocial")
            if razon_social not in saldos_current:
                # print(f"Querying database for {razon_social}, periodo: {periodo}, mes: {mes}")
