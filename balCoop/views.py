...
class BalCoopApiViewIndicador(APIView):
    def post(self, request):
        ...
        for item in data:
            ...
            # Determina la URL y el campo de cuenta según el periodo
            if periodo == 2020:
                base_url = "https://www.datos.gov.co/resource/78xz-k3hv.json?$limit=50000"
                campo_cuenta = 'codcuenta'
            elif periodo == 2021:
                base_url = "https://www.datos.gov.co/resource/irgu-au8v.json?$limit=50000"
                campo_cuenta = 'codrenglon'
            else:
                base_url = "https://www.datos.gov.co/resource/tic6-rbue.json?$limit=50000"
                campo_cuenta = 'codrenglon'

            puc_codes_current = ["100000", "110000", "120000", "140000", "210000", "230000", "240000", "300000", "310000", "311010", "320000", "330500", "340500", "350000", "415000", "615005", "615010", "615015", "615020", "615035"]
            puc_codes_current_str = ','.join(f"'{code}'" for code in puc_codes_current)

            # Obtiene los datos del periodo actual
            url_solidaria = f"{base_url}&$where=a_o='{periodo}' AND mes='{mes}' AND {campo_cuenta} IN ({puc_codes_current_str})"
            all_data = [] 
            response_solidaria = requests.get(url_solidaria)
            if response_solidaria.status_code == 200:
                all_data.extend(response_solidaria.json())
            ...
            print("(☞ﾟヮﾟ)☞☜(ﾟヮﾟ☜)"*10)
            print(f"Datos actuales (periodo {periodo}, mes {mes}): {len(all_data)}")
            ...
            for result in all_data:
                ...
                if cuenta in puc_codes_current:
                    saldos_current[nit][cuenta] += valor_en_pesos
            ...
            for nit_info in item.get("nit", {}).get("solidaria", []):
                ...
                if not any(saldos_current[razon_social].values()):
                    q_current_period = Q(entidad_RS=razon_social, periodo=periodo, mes=mes_number) & Q(puc_codigo__in=puc_codes_current)
                    query_results_current = (BalCoopModel.objects.filter(q_current_period)
                        .values("puc_codigo")
            ...
            if periodo_anterior != periodo_anterior_actual or mes_anterior != mes_ultimo_str:
                ...
                url_solidaria_prev = f"{base_url}&$where=a_o='{periodo_anterior_actual}' AND mes='{mes_12_anterior}' AND {campo_cuenta} IN ({puc_codes_prev_str})"

                response_solidaria_prev = requests.get(url_solidaria_prev)
                if response_solidaria_prev.status_code == 200:
                    all_data_prev.extend(response_solidaria_prev.json())

                print("(☞ﾟヮﾟ)☞☜(ﾟヮﾟ☜)"*10)
                print(f"Datos anteriores (periodo {periodo_anterior_actual}, mes {mes_12_anterior}): {len(all_data_prev)}")
            ...
            for result in all_data_prev:
                ...
                cuenta = result.get(campo_cuenta)
                valor_en_pesos = clean_currency_value_Decimal(result.get('valor_en_pesos', '0'))
                saldos_previous[nit][cuenta] += valor_en_pesos
            ...
            for nit_info in item.get("nit", {}).get("solidaria", []):
                ...
                if not any(saldos_previous[razon_social].values()):
                    q_previous_period = Q(entidad_RS=razon_social, periodo=periodo_anterior, mes=mes_ultimo) & Q(puc_codigo__in=puc_codes_prev)
                    query_results_previous = (BalCoopModel.objects.filter(q_previous_period)
                        .values("puc_codigo")
                        .annotate(total_saldo=Sum("saldo")))
...
class BalCoopApiViewIndicadorC(APIView):
    def post(self, request):
        ...
        for item in data:
            ...
            if periodo == 2020:
                baseUrl_entidadesSolidaria = "https://www.datos.gov.co/resource/78xz-k3hv.json?$limit=50000"
                campoCuenta = 'codcuenta'
            elif periodo == 2021:
                baseUrl_entidadesSolidaria = "https://www.datos.gov.co/resource/irgu-au8v.json?$limit=50000"
                campoCuenta = 'codrenglon'
            else:
                baseUrl_entidadesSolidaria = "https://www.datos.gov.co/resource/tic6-rbue.json?$limit=50000"
                campoCuenta = 'codrenglon'
            ...
            # Obtiene los datos del periodo actual
            url_solidaria = f"{baseUrl_entidadesSolidaria}&$where=a_o='{periodo}' AND mes='{mes}' AND {campoCuenta} IN ({puc_codes_current_str})"
            all_data = [] 
            response_solidaria = requests.get(url_solidaria)
            if response_solidaria.status_code == 200:
                all_data.extend(response_solidaria.json())
